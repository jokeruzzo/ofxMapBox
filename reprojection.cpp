...
void Projection::project_init(int proj)
{
    char buffer[32];
    Proj = proj;
    ...
    pj_source = pj_init_plus("+proj=sterea +lat_0=52.15616055555555 +lon_0=5.38763888888889 +k=0.9999079 +x_0=155000 +y_0=463000 +ellps=bessel +units=m +no_defs");
...
void Projection::project_exit(void)
{
    pj_free(pj_source);
    pj_source = NULL;
    pj_free(pj_target);
...
void Projection::reproject(double *lat, double *lon)
{
    double x[1], y[1], z[1];
 ...
 * variable map_width.
 */
void Projection::coords_to_tile(double *tilex, double *tiley, double lon, double lat)
{
    double x[1], y[1], z[1];
